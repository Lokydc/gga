...
module.exports = async (client, interaction, t) => {
    ...
    let responseImage = await ta.slap()
    const embed = new MessageEmbed()
    .setDescription(`<a:jujuba_dance:977848862331011072> **| ${interaction.user} ${t('commands:roleplay.slap')} ${user}!**`)
...
collector.on('collect', async i => {
	...
	if (i.customId === `dance${user.id}`) {
        let response = await ta.slap()
        const embed = new MessageEmbed()
    .setDescription(`<a:jujuba_dance:977848862331011072> **| ${user} ${t('commands:roleplay.slap')} ${interaction.user}!**`)
    .setImage(response)
    .setColor('#ED02B2')
